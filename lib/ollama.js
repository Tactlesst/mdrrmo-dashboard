...
async function getOllamaClient() {
  if (ollamaClientPromise) return ollamaClientPromise;

  ollamaClientPromise = (async () => {
    try {
      const pkgName = 'ollama';
      const mod = await import(pkgName);
      const Ollama = mod?.Ollama;
      if (!Ollama) {
        throw new Error('Ollama export not found');
      }
      return new Ollama({
        host: process.env.OLLAMA_HOST || 'http://localhost:11434',
      });
    } catch (err) {
      logger.warn('Ollama package not available:', err?.message || String(err));
      return null;
    }
  })();
